...
function wspra_get_status_message($code=200){
 ...
 function wspra_response($code='',$response,$_authintication,$data=array()){
            $_code = ($code)?$code:wspra_http_response_code();
            $opt = array('status' =>wspra_get_status_message($code),'response' => $response , 'code' => $code , 'data' => $data  );
            $create_log = wspra_create_api_log($_authintication,$opt);
            return $opt;
            exit;
 }
...
function wspra_create_api_log($ApiSecret,$response){
    // $_get_settings = esc_attr(wspra_get_settings('wpr_set_req_check'));
    // if($_get_settings == 0 ){
    //     return;
    // }
      global $wpdb;
      $tab = $wpdb->prefix.WSPRA_DB.'api_log';
